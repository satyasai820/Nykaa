...
import { GetBannerData,  } from "../Redux/Api/GetCardData";
import {GetBanners} from "../Redux/Actions/GetBanners";
import { useParams } from "react-router-dom";

const FaceFirstComponent = ({data, GetBanners}) => {
    ...
    const BannerData = async () => {
        const BannerImg = await GetBannerData();
        GetBanners(BannerImg);
        console.log("This is banner data -->", BannerImg);
        console.log('this is banner data of data variable--->', data);
    }

    
    const prams = useParams();
    const routes = prams.id;
    console.log('This is', routes, 'Page');
    const BannerImagesData = data;
    console.log("This is BannerImagesData in before filter", BannerImagesData)
    const BannerComponentImgs = BannerImagesData.filter( (item) => item.status === routes);
    console.log("this is",routes,'imgs ---->', BannerComponentImgs);
    // alert('hi')
                    ...
                    <Grid sx={{marginTop:'20px', backgroundColor:'#FFFFFF'}}>
                        ...
                        {BannerComponentImgs.map((item)=>(
                            <Grid sx={{marginRight:'10px'}}>
                              ...
                              <img style={{maxWidth:'100%', height:'auto', borderRadius:'8px'}} src={item.img2} alt="" />
                              </Grid>
                        ))}
                        </Grid>
...
const mapStateToProps = (state) => {
    return{
        data:state.Reducer2.Bannerdata,
    }
}
...
export default connect(mapStateToProps, mapDispatchToProps)(FaceFirstComponent);